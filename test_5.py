...
def test_get_user_2():
    r = requests.get("https://reqres.in/api/users/2", timeout=10)
...
def test_post_create_user():
    ...
    r = requests.post("https://reqres.in/api/users", json=payload, timeout=10)
...
def test_put_update_user():
    ...
    r = requests.put("https://reqres.in/api/users/2", json=payload, timeout=10)
    assert r.status_code == 200, f"Unexpected status: {r.status_code}"
    j = r.json()
